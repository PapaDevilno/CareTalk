//
//  StringResources.swift
//  CareTalk
//
//  Created by ichiro on 02/11/23.
//

import Foundation

struct StringResources {
    
    ///Tutorial
    //OnBoarding =================
    
    let greetings = "Halo, aku "
    let eva = "EVA"
    let evaDesc = "Aku adalah teman virtual yang siap membantu kamu berkomunikasi dengan orang-orang yang mengalami kesulitan berbicara akibat disartria"
    let continueTutorial = "Pelajari lebih lanjut"
    let skipTutorial = "Lewati tutorial"
    
    
    //OnBoarding - Gallery pt1 =================
    
    let galleryTitle = "Galeri"
    let galleryCongratulateFirst = "Selamat!"
    let galleryCongratulate = "Kamu telah berhasil menggunakan "
    let seeGallery = "kamu dapat melihat galeri rekaman sebelumnya dengan cara "
    let swipeLeft = "geser ke kiri"
    
    //OnBoarding - Gallery pt2
    
    let galleryDesc_First = "Berikut adalah hasil suara yang sudah direkam dan diinterpretasi sebelumnya"
    let galleryDesc_Second = "Ketuk rekaman tersebut untuk menampilkan hasilnya"
    
    //OnBoarding - Gallery pt3
    
    let galleryDesc_Third = "Lalu dapat memutar ulang audio yang direkam, membaca transkripsi, dan menghapus rekaman sebelumnya"
    let galleryDesc_Fourth = " untuk kembali ke layar interpretasi"
    let swipeRight = "Geser ke kanan"
    
    //OnBoarding - Record
    
    let instruction = "Tekan selama 2 detik\nuntuk mulai merekam"
    
    //OnBoarding - How To Record
    
    let howTo = "Cara menggunakan :"
    let remember = "Ingat:"
    let firstHowTo = "Ketuk di mana saja agar saya bisa mendengarkan."
    let secondHowTo = "Ucapkan kalimat yang ingin diinterpretasi oleh si pasien."
    let thirdHowTo = "Ketuk lagi agar saya bisa berhenti mendengarkan."
    let fourthHowTo = "Geser ke kiri untuk melihat semua rekaman Anda."
    let fifthHowTo = "Geser ke kanan untuk kembali ke      halaman perekaman."
    let repeatTutorial = "Ulangi tutorial"
    let close = "Tutup"
    
    //OnBoarding - Congratulations
    
    let congratulate = "Selamat!"
    let congratulateText = "Anda telah berhasil mempelajari "
    let finish = "Selesai"
    
    //OnBoarding - Tips
    
    let tips = "Tips"
    let tipsDesc = "Untuk memperjelas hasil interpretasi, aku menyarankan untuk memberikan Airpods pada pasien saat menggunakan "
    
    
}
